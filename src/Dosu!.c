...
#define MAX_TIMING_POINTS 100
#define MAX_CURVE_POINTS 20 // Zmenseno pro vyreseni array size too large
#define FPS 30
#define PRE_SHOW 2000 // ms pred tim, nez kruh ma byt aktivni
#define AUDIO_DELAY_MS 0 // nastav si v ms, napr. 200 pro posun audia
#define BUFFER_SIZE 2048 // Mensi buffer pro lepsi latenci/sync
#define CIRCLE_RADIUS 20
#define APPROACH_SCALE 3.0 // Pocatecni velikost approach kruhu (nasobek radia)
#define FOLLOW_RADIUS 10 // Velikost follow circle pro slidery
#define FADE_OUT_MS 250 // Nastavitelna hodnota v ms, kdy objekty zmizi po hitu/konci (snizeno o 50ms)
#define MAX_LIFE 100 // Maximalni zivoty hrace
#define LIFE_DRAIN_PER_SEC 2 // Kolik zivotu ubyde za sekundu (jako v osu!)
#define LIFE_GAIN_300 5 // Prirustek zivotu za 300 bodu
#define LIFE_LOSS_50 3 // Ubitek zivotu za 50 bodu
#define LIFE_LOSS_MISS 10 // Ubitek zivotu za chybu
#define HIT_WINDOW_300 132 // ms pro 300 bodu
#define HIT_WINDOW_100 230 // ms pro 100 bodu
#define HIT_WINDOW_50 329 // ms pro 50 bodu
#define MISS_WINDOW 428 // ms pro miss
#define DIRECTION_TOLERANCE 0.5 // Kosinus uhlu pro naznaceni smeru (0.5 ~ 60 stupnu)
...
typedef struct {
    long time;
    double beatLength;
    int inherited; // 0 pro uninherited, 1 pro inherited
} TimingPoint;
...
void getFollowPosition(int x, int y, int curvePoints[][2], int curveCount, float progress, int *targetX, int *targetY) {
    ...
    for (i = 0; i < pathCount - 1; i++) {
        if (cum + segLengths[i] >= targetDist) {
            ...
            *targetX = pathPoints[i][0] + (int)(frac * (pathPoints[i + 1][0] - pathPoints[i][0]));
            *targetY = pathPoints[i][1] + (int)(frac * (pathPoints[i + 1][1] - pathPoints[i][1]));
            return;
        }
        cum += segLengths[i];
    }
...
// ----------------------------------------
// Kresleni follow circle
void drawFollowCircle(int x, int y, int points[][2], int count, char curveType, float progress, int repeats, long now, long startTime) {
    int targetX, targetY;
    getFollowPosition(x, y, points, count, progress, &targetX, &targetY);
    setcolor(YELLOW);
    circle(targetX, targetY, FOLLOW_RADIUS);
}
...
int checkDirection(int sliderX, int sliderY, int nextX, int nextY, int mouseDX, int mouseDY) {
    int sliderDX;
    int sliderDY;
    double sliderMag;
    double mouseMag;
...
void showEndScreen() {
    ...
    cleardevice();
    setcolor(WHITE);
    outtextxy(10, 10, "Konec hry - statistiky:");
    sprintf(buf, "300: %d", count300);
    outtextxy(10, 50, buf);
    sprintf(buf, "100: %d", count100);
    outtextxy(10, 70, buf);
    sprintf(buf, "50: %d", count50);
    outtextxy(10, 90, buf);
    sprintf(buf, "Chyby: %d", countMiss);
    outtextxy(10, 110, buf);
    sprintf(buf, "Celkove skore: %d", score);
    outtextxy(10, 130, buf);
    sprintf(buf, "Presnost: %.2f%%", accuracy);
    outtextxy(10, 150, buf);
    outtextxy(10, 170, "Stiskni libovolnou klavesu pro pokracovani...");
    getch();
}
        ...
        if (inBreak) {
            ...
            for (i = 0; i < objectCount; i++) {
                ...
                if (objects[i].type == 1) { // Circle
                    ...
                    durationOne = totalDuration / objects[i].repeats;
                    disappearTime = objects[i].time + (long)totalDuration + FADE_OUT_MS;
                    if (now > disappearTime) continue;
                    ...
                    if (now >= objects[i].time) {
                        ...
                        drawFollowCircle(objects[i].x, objects[i].y, objects[i].curvePoints, objects[i].curvePointCount, objects[i].curveType, localProgress, objects[i].repeats, now, objects[i].time);
                    }
                }
                active++;
            }
