...
#define FPS 30
#define PRE_SHOW 3000 // ms pred tim, nez kruh ma byt aktivni
...
void loadBeatmap(const char* filename) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strstr(line, "|")) { // Slider
            res = sscanf(line, "%d,%d,%ld,%d,%d,%[^,],%d,%lf", &x, &y, &t, &type, &hitsound, curve, &repeats, &length);
            if (res >= 7) {
    ...
    while (!kbhit() && !done) {
        ...
        if (inBreak) {
            ...
            for (i = 0; i < objectCount; i++) {
                ...
                disappearTime = objects[i].time + FADE_OUT_MS; // Default pro circle
                ...
                if (objects[i].type == 1) { // Circle
                    ...
                    // Kruh a cislo jen po approach (po hitu kratce)
                    if (now <= objects[i].time + FADE_OUT_MS) {
                        setcolor(GREEN);
        ...
        // Check if all objects done
        if (objectCount > 0 && now > objects[objectCount-1].time + 1000) {
            done = 1;
        }
